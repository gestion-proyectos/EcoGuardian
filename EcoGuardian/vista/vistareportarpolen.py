...
def vista_reportarpolen():
    ...
    if request.method == 'POST':
        try:
            ...
            tipo_evento = "Polen"  # Valor por defecto
            ...
            if not ubicacion:
                ...
                return redirect(url_for('idvistareportarpolen.vista_reportarpolen'))
            ...
            if conexion.ejecutarComandoSql(sql, parametros):
                flash('Reporte de polen registrado exitosamente', 'success')
                return redirect(url_for('idvistareportarpolen.vista_reportarpolen'))
            else:
                flash('Error al registrar el reporte de polen', 'error')
