...
def vista_verincenidos():
    ...
    if conn is None:
        ...
        return render_template('ver_incendios.html', reportes=[])

    # Primero, verificar si hay datos en la tabla
    sql_check = """
        SELECT COUNT(*) as total 
        FROM reportes_ambientales 
        WHERE tipo_evento = 'Incendio';
    """
    count_result = control_con.ejecutarSelect(sql_check)
    print("Número total de incendios en la base de datos:", count_result[0]['total'] if count_result else 0)
    ...
    return render_template('ver_incendios.html', reportes=reportes)